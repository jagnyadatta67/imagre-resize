...
def reprocess_single_image(
    ...
    """
    Reprocess a single image WITH Vision AI for bbox-aware processing.
    Method (gen_fill / auto / fill / pillow) always comes from user input.

    Flow
    ----
    1. Vision AI  → detect objects + bbox
    2. bbox used  → guide crop position / dimensions
    3. method     → defines HOW to process (pad bg, fill, pillow)

    Returns
    -------
    (output_bytes, used_cloudinary, cloudinary_url)
    """
    # ── Step 1: Vision AI — get object bbox ──────────────────────
    _, objects, _, _ = _vision_analyze(image_bytes, logger)
    bbox = _detect_bbox(objects, logger)

    # ── Step 2: Apply user-chosen method with bbox awareness ─────
    return _reprocess_cloudinary(image_bytes, filename, sku_id, method, bbox, logger)


def _reprocess_cloudinary(
    image_bytes: bytes,
    filename:    str,
    sku_id:      str,
    method:      str,
    ...
    """
    Dedicated reprocess path — bbox-aware, method driven by user input.
    Fully independent of the worker pipeline.

    method → behaviour
    ──────────────────────────────────────────────────────────────
    pillow    Pillow crop centred on bbox object (fallback: centre crop)
    fill      Cloudinary FILL  gravity:auto
    auto      Cloudinary PAD   background:auto   gravity:center
    gen_fill  Cloudinary PAD   background:gen_fill  gravity:center
    """
    img      = Image.open(io.BytesIO(image_bytes))
    img_w, img_h  = img.size
    target_width  = int(img_h * TARGET_RATIO)

    # ── pillow: use bbox center for object-aware crop ────────────
    if method == "pillow":
        if bbox:
            ...
            logger.info(
                f"[{filename}] reprocess pillow → bbox crop  "
                f"cx={obj_cx}  left={left}  right={right}"
            )
            return _pil_to_bytes(img.crop((left, 0, right, img_h))), False, None
        else:
            logger.info(f"[{filename}] reprocess pillow → no bbox detected, centre crop")
    ...
    if method == "fill":
        ...
        logger.info(f"[{filename}] reprocess → Cloudinary FILL  gravity:auto  bbox={'yes' if bbox else 'none'}")
    ...
    # ── auto / gen_fill: Cloudinary PAD with chosen background ───
    if method in ("auto", "gen_fill"):
        bg = method    # "auto" or "gen_fill" — valid Cloudinary background values
        transformation = [{"width": TARGET_W, "height": TARGET_H,
                           "crop": "pad", "gravity": "center",
                           "background": bg}]
        logger.info(
            f"[{filename}] reprocess → Cloudinary PAD  "
            f"background:{bg}  bbox={'yes' if bbox else 'none'}"
        )
        out_bytes, used, url = _cloudinary_upload_and_fetch(
            image_bytes, sku_id, filename, transformation, logger
        )
    ...
    logger.warning(f"[{filename}] reprocess → unknown method '{method}', falling back to Pillow centre crop")
    return _pil_center_crop(img), False, None
